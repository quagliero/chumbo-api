...
const getManager = (year, week, teamId) =>
  r.find((manager) => {
...
const parseWeek = (year, week, teams = {}) =>
  new Promise((resolve) => {
    rp(`${urlBase}/${year}/schedule?scheduleDetail=${week}`).then((html) => {
      ...
      $matchups.each((i, el) => {
        ...
        const manager1 = getManager(
          year,
          week,
          $team1.attr("class").split("teamId-")[1]
        );
        const manager2 = getManager(
          year,
          week,
          $team2.attr("class").split("teamId-")[1]
        );
        ...
        [manager1, manager2].forEach((m) => {
          ...
          parseTeamRoster(teamId, year, week).then((data) => {
            ...
            if (count === matchups.length * 2) {
              fs.writeFile(
                `../raw_data/${year}/matchups/${week}.json`,
                JSON.stringify(sleeper, null, 2),
                "utf-8",
                (err) => {
                  if (err) throw err;
                }
              ...
              console.log(
                `${year} week ${week} matchups written to ../raw_data/${year}/matchups/${week}.json`
              );
            }
          });
        });
...
const parseRegularSeasonStandings = (year) =>
  new Promise((resolve) => {
    spinner.info(`Fetching ${year} standings`);
    rp(`${urlBase}/${year}/standings?historyStandingsType=regular`).then(
      (html) => {
        const standingsData = {};
        ...
        const divisions = $("#leagueHistoryStandings .tableWrap h5").map(
          (i, el) => {
        ...
        $standings.each((i, el) => {
          const $el = $(el);
          const teamHref = $el
            .find(".teamImageAndName a.teamName")
            .attr("href");
          ...
          managerQueue
            ...
            .then((mId) => {
              const manager = r.find(
                r.propSatisfies(r.contains(mId), "userId"),
                managers
              );
...
const parseTeamRoster = (team, year, week) =>
  new Promise((resolve) => {
    rp(
      `${urlBase}/${year}/teamhome?teamId=${team}&week=${week}&statWeek=${week}&statType=weekStats`
    ).then((html) => {
      const $ = cheerio.load(html);
      ...
      $roster.each((i, el) => {
        if (
          el.attribs.class.indexOf("odd") > -1 ||
          el.attribs.class.indexOf("even") > -1
        ) {
...
const rosterHistory = (yrs, team, title) => {
  ...
  yrs.forEach((year) => {
    r.range(1, 17).forEach((week) => {
      ...
      queue
        .add(() => parseTeamRoster(team, year, week))
        .then((data) => {
...
const scrape = (yrs, title, tiers) => {
  ...
  yrs.forEach((year) => {
    queue
      .add(() => parseRegularSeasonStandings(year))
      .then((data) => {
        history[year] = {
          standings: data,
        };
      ...
      .then(() => {
        ...
        r.range(1, year >= 2014 ? 14 : 15).forEach((week) => {
          ...
          queue
            .add(() => parseWeek(year, week, history[year].standings))
            .then((data) => {
  ...
  queue.onIdle().then(() => {
      ...
      r.filter((team) => {
        ...
        if (tiers) {
          return [
            ...
            "jimmie",
            "brock",
            "Disciples of Aaron Rodgers",
            "chris",
            "Suh Suh Sudio",
          ].every((tm) => team.name !== tm);
        }
    ...
    const tableData = r.map(r.pipe(r.values, r.remove(4, 1)), sorted);
      ...
      [
        {
          ...
          formatter: (value) => {
            ...
            if (manager.crowns && !tiers) {
              value = manager.teamName.concat(
                ` ${"🏆".repeat(manager.crowns || 0)}`
              );
            }
...
const allWeeks = () =>
  years.map((year) =>
    r.range(1, year >= 2014 ? 14 : 15).map((week) => {
      if (year === years[years.length - 1] && week >= currentWeek) {
        return null;
      }
...
const getWeek = (week) =>
  years.map((year) => {
    ...
    if (
      (year >= 2014 && week >= 14) ||
      (year === years[years.length - 1] && week >= currentWeek)
    ) {
      return [];
    }
...
const getWeekRecord = (week) =>
  Promise.all(getWeek(week)).then((years) => {
...
const getHeadToHead = (a, b) =>
  Promise.all(r.filter(r.identity, r.flatten(allWeeks()))).then((gameWeeks) => {
    const results = [];
    const details = {
      [a]: {
        wins: 0,
    ...
    results.forEach((game) => {
      console.log(
        `${game[0][0]} ${game[0][1]} - ${game[1][1]} ${game[1][0]} (Week ${game[2].week}, ${game[2].year})`
      );
    });
...
const parsePicks = (year) =>
  new Promise((resolve) => {
    rp(
      `${urlBase}/${year}/draftresults?draftResultsDetail=0&draftResultsTab=round&draftResultsType=results`
    ).then((html) => {
      const $ = cheerio.load(html);
      ...
      $rounds.each((i, round) => {
        ...
        $picks.each((k, pick) => {
          ...
          const manager = getManager(
            year,
            1,
            $pick.find(".teamName").attr("class").split("teamId-")[1]
          );
      ...
      console.log(
        `${year} draft picks written to ../raw_data/${year}/picks.json`
      );
    ...
    rp(`${urlBase}/${year}/owners`).then((html) => {
      ...
      $owners.each((i, el) => {
        ...
        const manager = getManager(
          year,
          16,
          $team.attr("class").split("teamId-")[1]
        );
  ...
  new Promise(() => {
    ...
    for (let i = 1; i <= count; i++) {
      rosters.push(
        ...
        new Promise((resolve) =>
          parseTeamRoster(i, year, 16).then((roster) => {
            new Promise((resultResolve) => {
              rp(
                `${urlBase}/${year}/standings?historyStandingsType=regular`
              ).then((html) => {
                const $ = cheerio.load(html);

                const $team = $(
                  `#leagueHistoryStandings .tableType-team > tbody > tr.team-${i}`
                );
                const record = $team.find(".teamRecord").html().split("-");
                const points = $team.find(".teamPts").first().text().split(".");
                const points_against = $team
                  .find(".teamPts.last")
                  .text()
                  .split(".");
                ...
                const fpts_against = parseFloat(
                  points_against[0].replace(/,/g, "")
                );
              ...
              new Promise((rosterAndResultResolve) => {
                rp(`${urlBase}/${year}/owners`).then((html) => {
                  ...
                  const $team = $(
                    `#leagueOwners .tableType-team > tbody > tr.team-${i}`
                  );
                  const moves = Number(
                    $team.find(".teamTransactionCount").text()
                  );
                ...
                new Promise(() => {
                  rp(
                    `${urlBase}/${year}/schedule?standingsTab=schedule&scheduleType=team&scheduleDetail=${i}`
                  ).then((html) => {
                    const $ = cheerio.load(html);
                    const $weeks = $(
                      "#scheduleSchedule .tableType-weeks > tbody > tr"
                    );
                    ...
                    $weeks.each((i, el) => {
                      ...
                      if (i + 1 <= (year >= 2014 ? 13 : 14)) {
                        record.push($(el).find(".resultText").text().charAt(0));
                      }
                    });
    ...
    Promise.all(rosters).then((data) => {
      ...
      console.log(
        `${year} rosters written to ../raw_data/${year}/rosters.json`
      );
    });
  });
...
const parseSettings = (year) =>
  new Promise((resolve) => {
    rp(`${urlBase}/${year}/settings`).then((html) => {
      ...
      console.log(
        `${year} league settings written to ../raw_data/${year}/league.json`
      );
      resolve(league);
    });
  });
...
const parseBracket = (year, consolation) =>
  new Promise((resolve) => {
    ...
    rp(
        ...
        $matchups.each((k, el) => {
          ...
          if (!isBye) {
            ...
            const manager1 = getManager(
              year,
              16,
              $team1.find(".teamName").attr("class")?.split("teamId-")[1]
            );
            const manager2 = getManager(
              year,
              16,
              $team2.find(".teamName").attr("class")?.split("teamId-")[1]
            );
            let winner, loser;
      ...
      console.log(
        `${year} ${bracketType} bracket written to ../raw_data/${year}/${fileName}.json`
      );
      resolve(bracket);
    });
  });
...
const buildDraft = (year) =>
  new Promise((resolve) => {
    ...
    console.log(
      `${year} draft order written to ../raw_data/${year}/draft.json`
    );
